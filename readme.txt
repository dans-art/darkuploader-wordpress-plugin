=== DarkWP - Image uploader for Darktable ===
...
DarkWP exposes a small REST API (`darkwp/v1`) that lets Darktable, or any
compatible client, upload exported images directly into WordPress and route
them into a supported gallery plugin.

`GET /darkwp/v1/info` lists the gallery plugins that are installed and
active, along with the upload-form fields each one accepts.

`POST /darkwp/v1/media` uploads a single image to the gallery named in the
`target` field. An recommended `X-Darkwp-Batch` request header lets several
...
= 0.2.0 - 2026-08-26 =
* Added the `/darkwp/v1/media` REST endpoint for uploading an image to a target gallery.
...
* Added batch support (`X-Darkwp-Batch` header) so multiple images from one Darktable export can share a single newly created gallery.
